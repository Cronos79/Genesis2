...
bool GenSprite::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, float width, float height, std::string spritePath, ConstantBuffer<CB_VS_vertexshader_2d>& cb_vs_vertexshader_2d)
{
	...
	texture = std::make_unique<GenTexture>(device, spritePath, aiTextureType::aiTextureType_DIFFUSE);
	...
	GENWND_ERROR_IF_FAILED(hr, "Failed to initialize vertex buffer for sprite.");
...
void GenSprite::Draw(XMMATRIX orthoMatrix)
{
...
float GenSprite::GetWidth()
{
	return scale.x;
}

float GenSprite::GetHeight()
{
	return scale.y;
}

void GenSprite::UpdateMatrix()
{
